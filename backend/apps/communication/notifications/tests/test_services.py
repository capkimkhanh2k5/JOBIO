...
class NotificationServicesTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.inactive_notification_type = NotificationType.objects.create(
            type_name="deprecated", template="Deprecated notification", is_active=False
        )
    ...
    def test_create_notification_invalid_type(self):
        ...
        data = NotificationCreateInput(
            notification_type_id=99999, title="Test", content="Content"
        )
    ...
    def test_send_bulk_notifications_success(self):
        ...
        user_notification = Notification.objects.filter(
            user=self.user, title="Announcement"
        ).first()
        other_notification = Notification.objects.filter(
            user=self.other_user, title="Announcement"
        ).first()
