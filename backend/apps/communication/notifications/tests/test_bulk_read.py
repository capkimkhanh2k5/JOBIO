...
class NotificationBulkReadTest(TestCase):
    ...
    def test_mark_all_read(self):
        ...
        response = self.client.patch(
            "/api/notifications/mark-read/", {"read_all": True}
        )
    ...
    def test_mark_specific_ids_read(self):
        ...
        response = self.client.patch(
            "/api/notifications/mark-read/", {"notification_ids": ids_to_read}
        )
        ...
        self.assertEqual(
            Notification.objects.filter(user=self.user, is_read=False).count(), 3
        )
