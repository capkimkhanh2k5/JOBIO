...
def create_notification(
    user: CustomUser, data: NotificationCreateInput
) -> Notification:
    """
    ...
    try:
        notification_type = NotificationType.objects.get(
            id=data.notification_type_id, is_active=True
        )
...
def mark_all_as_read(user_id: int) -> int:
    ...
    """
    updated_count = Notification.objects.filter(user_id=user_id, is_read=False).update(
...
def bulk_mark_as_read(notification_ids: list[int], user_id: int) -> int:
    ...
    updated_count = Notification.objects.filter(
        id__in=notification_ids, user_id=user_id, is_read=False
...
def send_notification(
    ...
    try:
        ...
        notification_type = NotificationType.objects.get(
            type_name=notification_type_name, is_active=True
        )
...
def notify_admins(
    ...
    """
    admin_ids = list(
        CustomUser.objects.filter(role="admin", is_active=True).values_list(
            "id", flat=True
        )
    )
