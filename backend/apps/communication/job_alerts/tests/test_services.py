...
class JobMatchingServiceTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.company = Company.objects.create(
            user=cls.employer_user, company_name="Tech Corp", slug="tech-corp"
        )
        ...
        cls.hanoi = Province.objects.create(
            province_name="Ha Noi", region="north", province_type="municipality"
        )
        cls.hcm = Province.objects.create(
            province_name="Ho Chi Minh", region="south", province_type="municipality"
        )

        cls.addr_hanoi = Address.objects.create(
            address_line="123 Hanoi St", province=cls.hanoi
        )
        cls.addr_hcm = Address.objects.create(
            address_line="456 HCM St", province=cls.hcm
        )

        cls.skill_category = SkillCategory.objects.create(
            name="Programming", slug="programming"
        )

        # CORRECTED: Create SKILL objects first
        cls.skill_python = Skill.objects.create(
            name="Python", slug="python", category=cls.skill_category
        )
        cls.skill_django = Skill.objects.create(
            name="Django", slug="django", category=cls.skill_category
        )
        cls.skill_react = Skill.objects.create(
            name="React", slug="react", category=cls.skill_category
        )
