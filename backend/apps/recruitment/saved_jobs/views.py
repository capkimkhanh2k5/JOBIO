...
from .selectors.saved_jobs import (
    list_saved_jobs_by_recruiter,
    get_saved_job_by_id,
    get_saved_job_by_recruiter_and_job,
    is_job_saved,
)
from .services.saved_jobs import (
    save_job,
    unsave_job,
    update_saved_job,
    SavedJobUpdateInput,
...
class SavedJobViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request):
        ...
        return Response(
            SavedJobSerializer(saved_job).data, status=status.HTTP_201_CREATED
        )
    ...
    def partial_update(self, request, pk=None):
        ...
        if not saved_job:
            return Response(
                {"detail": "Saved job not found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def folders(self, request):
        ...
        folder_names = (
            SavedJob.objects.filter(recruiter=recruiter, folder_name__isnull=False)
            .exclude(folder_name="")
