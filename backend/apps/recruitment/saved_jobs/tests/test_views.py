...
class SavedJobViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.employer = CustomUser.objects.create_user(
            email="employer@example.com", password="password123", full_name="Employer"
        )
    ...
    def test_list_saved_jobs_success(self):
        ...
        SavedJob.objects.create(
            recruiter=self.recruiter, job=self.job, folder_name="Tech Jobs"
        )
    ...
    def test_update_saved_job_success(self):
        ...
        saved = SavedJob.objects.create(recruiter=self.recruiter, job=self.job)

        self.client.force_authenticate(user=self.user)
        url = f"/api/saved-jobs/{saved.id}/"
        data = {"folder_name": "Tech Jobs", "notes": "Interesting position"}
    ...
    def test_save_job_success(self):
        ...
        self.assertTrue(
            SavedJob.objects.filter(recruiter=self.recruiter, job=self.job).exists()
        )
    ...
    def test_save_job_already_saved(self):
        ...
        # Save job first
        SavedJob.objects.create(recruiter=self.recruiter, job=self.job)
    ...
    def test_folder_organization(self):
        ...
        response1 = self.client.post(
            f"/api/jobs/{self.job.id}/save/", {"folder_name": "AI Jobs"}
        )
        self.assertEqual(response1.status_code, status.HTTP_201_CREATED)

        # Save job2 to folder B
        response2 = self.client.post(
            f"/api/jobs/{self.job2.id}/save/", {"folder_name": "Backend Jobs"}
        )
