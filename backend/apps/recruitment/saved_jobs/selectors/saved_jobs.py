...
def list_saved_jobs_by_recruiter(recruiter_id: int) -> QuerySet[SavedJob]:
    ...
    """
    return (
        SavedJob.objects.filter(recruiter_id=recruiter_id)
        .select_related("job", "job__company", "job__address", "job__address__province")
        .order_by("-created_at")
    )
...
def get_saved_job_by_recruiter_and_job(
    recruiter_id: int, job_id: int
) -> Optional[SavedJob]:
    """
...
def is_job_saved(recruiter_id: int, job_id: int) -> bool:
    ...
    """
    return SavedJob.objects.filter(recruiter_id=recruiter_id, job_id=job_id).exists()
