...
class JobListSerializer(serializers.ModelSerializer):
    ...
    company_slug = serializers.CharField(
        source="company.slug", read_only=True, allow_null=True
    )
    logo_url = serializers.CharField(
        source="company.logo_url", read_only=True, allow_null=True
    )
    ...
    applications_count = serializers.IntegerField(
        source="application_count", read_only=True
    )
...
class JobCreateSerializer(serializers.Serializer):
    ...
    title = serializers.CharField(
        max_length=255, required=False, allow_blank=True, default="Tin tuyển dụng mới"
    )
    ...
    salary_type = serializers.ChoiceField(
        choices=["monthly", "yearly", "hourly", "project"],
        required=False,
        default="monthly",
    )
...
class JobUpdateSerializer(serializers.Serializer):
    ...
    seo_keywords = serializers.ListField(child=serializers.CharField(), required=False)
    is_remote = serializers.BooleanField(required=False)
    application_deadline = serializers.DateField(required=False, allow_null=True)
    status = serializers.ChoiceField(
        choices=["draft", "published", "closed", "expired"], required=False
    )
...
class JobStatusSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        choices=["draft", "published", "closed", "expired"], required=True
    )
