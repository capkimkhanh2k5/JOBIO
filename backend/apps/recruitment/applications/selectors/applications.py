...
def get_application_stats(user) -> dict:
    ...
    return {"total": total, "by_status": by_status, "recent_7_days": recent_7_days}


def list_applications_for_export(
    user, job_id: int = None, status: str = None
) -> QuerySet[Application]:
    """
    ...
    owned_jobs = Job.objects.filter(company__user=user)
    queryset = Application.objects.filter(job__in=owned_jobs).select_related(
...
def list_applications_by_rating(
    job_id: int, rating: int = None, min_rating: int = None, max_rating: int = None
) -> QuerySet[Application]:
    """
