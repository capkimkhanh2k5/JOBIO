...
class ApplicationViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.recruiter = Recruiter.objects.create(
            user=self.applicant_user, bio="A software engineer"
        )
    ...
    def test_create_application_with_cover_letter(self):
        ...
        url = "/api/applications/"
        data = {"job_id": self.job.id, "cover_letter": "This is my cover letter"}
        response = self.client.post(url, data)
    ...
    def test_change_status_pending_company_forbidden(self):
        ...
        pending_application = Application.objects.create(
            job=pending_job, recruiter=pending_recruiter, status="pending"
        )
    ...
    def test_withdraw_already_withdrawn(self):
        ...
        app = Application.objects.create(
            job=self.job, recruiter=self.recruiter, status="withdrawn"
        )
    ...
    def test_bulk_action_success(self):
        ...
        data = {"application_ids": [app1.id, app2.id], "action": "shortlist"}
...
class JobApplicationFilterViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        # Company and Job
        self.company = Company.objects.create(
            user=self.job_owner, company_name="Test Company", description="Test"
        )
        self.job = Job.objects.create(
            company=self.company,
            title="Test Job",
    ...
    def test_by_rating_min_max(self):
        ...
        url = (
            f"/api/jobs/{self.job.id}/applications/by-rating/?min_rating=3&max_rating=5"
        )
        response = self.client.get(url)
