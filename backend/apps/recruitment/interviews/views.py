...
class InterviewViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request):
        ...
        try:
            application = Application.objects.select_related("job__company").get(
    ...
    def retrieve(self, request, pk=None):
        ...
        if not self._is_job_owner(request, interview) and not self._is_applicant(
            request, interview
        ):
            return Response(
                {"detail": "Permission denied"}, status=status.HTTP_403_FORBIDDEN
    ...
    def update(self, request, pk=None):
        ...
        permission_error = self._ensure_verified_company(
            request, interview.application.job.company
        )
        if permission_error:
            return permission_error
    ...
    def cancel(self, request, pk=None):
        ...
        try:
            updated = cancel_interview(
                interview, serializer.validated_data.get("reason")
            )
            return Response(InterviewDetailSerializer(updated).data)
        except ValueError as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
