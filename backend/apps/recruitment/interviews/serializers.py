...
def _infer_interview_mode(interview):
    ...
    if any(
        ...
        for keyword in ["onsite", "offline", "trực tiếp", "tại công ty"]
    ):
        return "onsite"
...
class InterviewDisplayMixin(serializers.ModelSerializer):
    ...
    company_id = serializers.IntegerField(
        source="application.job.company.id", read_only=True
    )
    company_name = serializers.CharField(
        source="application.job.company.company_name", read_only=True
    )
    company_slug = serializers.CharField(
        source="application.job.company.slug", read_only=True, allow_null=True
    )
    company_logo = serializers.CharField(
        source="application.job.company.logo_url", read_only=True, allow_null=True
    )
    interview_type_name = serializers.CharField(
        source="interview_type.name", read_only=True
    )
    ...
    interviewer_name = serializers.CharField(
        source="interviewer.full_name", read_only=True, allow_null=True
    )
    interviewer_avatar = serializers.CharField(
        source="interviewer.avatar_url", read_only=True, allow_null=True
    )
...
class InterviewListSerializer(InterviewDisplayMixin):
    ...
    applicant_name = serializers.CharField(
        source="application.recruiter.user.full_name", read_only=True
    )
    applicant_avatar = serializers.URLField(
        source="application.recruiter.user.avatar_url", read_only=True, allow_null=True
    )
    candidate_avatar = serializers.URLField(
        source="application.recruiter.user.avatar_url", read_only=True, allow_null=True
    )
...
class InterviewDetailSerializer(InterviewDisplayMixin):
    ...
    applicant_email = serializers.CharField(
        source="application.recruiter.user.email", read_only=True
    )
...
class InterviewUpdateSerializer(serializers.Serializer):
    ...
    result = serializers.ChoiceField(
        choices=["pass", "fail", "pending"], required=False
    )
...
class InterviewCompleteSerializer(serializers.Serializer):
    ...
    result = serializers.ChoiceField(choices=["pass", "fail"], required=True)
    feedback = serializers.CharField(required=False, allow_null=True, allow_blank=True)
    rating = serializers.IntegerField(
        required=False, allow_null=True, min_value=1, max_value=5
    )
