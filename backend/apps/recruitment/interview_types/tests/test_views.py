...
class InterviewTypeViewSetTests(TestCase):
    ...
    def setUp(self):
        ...
        self.interview_type = InterviewType.objects.create(
            name="Phone Interview", description="Initial phone screening"
        )
    ...
    def test_create_interview_type_user_forbidden(self):
        ...
        self.client.force_authenticate(user=self.user)
        data = {"name": "New Type", "description": "Test"}
        response = self.client.post("/api/interview-types/", data)
    ...
    def test_update_interview_type_admin(self):
        ...
        self.client.force_authenticate(user=self.admin)
        data = {"name": "Updated Phone Interview", "description": "Updated description"}
        response = self.client.put(
            f"/api/interview-types/{self.interview_type.id}/", data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_delete_interview_type_admin(self):
        ...
        self.assertFalse(
            InterviewType.objects.filter(id=self.interview_type.id).exists()
        )
