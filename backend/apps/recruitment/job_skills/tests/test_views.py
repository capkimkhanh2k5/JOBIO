...
class JobSkillViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.skill = Skill.objects.create(
            name="Python", slug="python", category=self.category
        )
        self.skill2 = Skill.objects.create(
            name="Django", slug="django", category=self.category
        )
    ...
    def test_create_job_skill_invalid_proficiency(self):
        ...
        url = f"/api/jobs/{self.job.id}/skills/"
        data = {"skill_id": self.skill.id, "proficiency_level": "invalid-level"}
        response = self.client.post(url, data)
    ...
    def test_update_job_skill_success(self):
        ...
        job_skill = JobSkill.objects.create(
            job=self.job, skill=self.skill, is_required=True, proficiency_level="basic"
        )
