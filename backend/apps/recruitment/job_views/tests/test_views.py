...
class JobViewAnalyticsTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.viewer = CustomUser.objects.create_user(
            email="viewer@example.com", password="password123", full_name="Job Viewer"
        )
    ...
    def test_get_view_stats_success(self):
        ...
        # Create some views
        JobView.objects.create(job=self.job, user=self.viewer, ip_address="192.168.1.1")
    ...
    def test_view_stats_counts_correct(self):
        ...
        # Create views - today
        JobView.objects.create(job=self.job, user=self.viewer, ip_address="192.168.1.1")
        JobView.objects.create(job=self.job, user=None, ip_address="192.168.1.2")

        # Create view from 3 days ago (within week)
        old_view = JobView.objects.create(
            job=self.job, user=None, ip_address="192.168.1.3"
        )
        JobView.objects.filter(id=old_view.id).update(viewed_at=now - timedelta(days=3))
    ...
    def test_demographics_auth_ratio(self):
        ...
        # Anonymous
        JobView.objects.create(job=self.job, user=None, ip_address="192.168.1.2")
        JobView.objects.create(job=self.job, user=None, ip_address="192.168.1.3")
    ...
    def test_record_view_updates_stats(self):
        ...
        JobView.objects.create(job=self.job, user=self.owner, ip_address="192.168.1.1")
    ...
    def test_unique_views_by_ip(self):
        ...
        # Same IP views twice
        JobView.objects.create(job=self.job, user=None, ip_address="192.168.1.1")
        JobView.objects.create(
            job=self.job,
            user=None,
    ...
    def test_referrer_parsing(self):
        ...
        self.assertTrue(any("google" in r for r in referrers))
