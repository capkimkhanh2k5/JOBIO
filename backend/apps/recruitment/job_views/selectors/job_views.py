...
def get_view_stats(job_id: int) -> dict:
    ...
    unique_by_ip = (
        queryset.filter(user__isnull=True).values("ip_address").distinct().count()
    )
...
def get_view_chart_data(job_id: int, period: str = "7d") -> dict:
    ...
    queryset = (
        JobView.objects.filter(job_id=job_id, viewed_at__date__gte=start_date)
...
def get_viewer_demographics(job_id: int) -> dict:
    ...
    return {
        ...
        "by_device": by_device,
        "authenticated_ratio": {"logged_in": logged_in, "anonymous": anonymous},
    }
