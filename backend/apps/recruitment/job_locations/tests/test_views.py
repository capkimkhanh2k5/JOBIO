...
class JobLocationViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.owner = CustomUser.objects.create_user(
            email="owner@example.com", password="password123", full_name="Job Owner"
        )
        ...
        self.province = Province.objects.create(
            province_name="Hà Nội", province_type="municipality", region="north"
        )
        self.commune = Commune.objects.create(
            commune_name="Cầu Giấy", commune_type="ward", province=self.province
        )

        # Create addresses
        self.address = Address.objects.create(
            address_line="123 Đường ABC", province=self.province, commune=self.commune
        )
        self.address2 = Address.objects.create(
            address_line="456 Đường XYZ", province=self.province, commune=self.commune
        )
    ...
    def test_list_job_locations_success(self):
        ...
        JobLocation.objects.create(job=self.job, address=self.address, is_primary=True)
    ...
    def test_update_job_location_success(self):
        ...
        job_location = JobLocation.objects.create(
            job=self.job, address=self.address, is_primary=False
        )

        self.client.force_authenticate(user=self.owner)
        url = f"/api/jobs/{self.job.id}/locations/{job_location.id}/"
        data = {"address_id": self.address2.id, "is_primary": True}
    ...
    def test_update_job_location_unauthenticated(self):
        ...
        job_location = JobLocation.objects.create(job=self.job, address=self.address)
    ...
    def test_update_job_location_wrong_job(self):
        ...
        job_location = JobLocation.objects.create(job=other_job, address=self.address)
    ...
    def test_multiple_locations_per_job(self):
        ...
        response1 = self.client.post(
            url, {"address_id": self.address.id, "is_primary": True}
        )
        self.assertEqual(response1.status_code, status.HTTP_201_CREATED)

        # Add second location
        response2 = self.client.post(
            url, {"address_id": self.address2.id, "is_primary": False}
        )
