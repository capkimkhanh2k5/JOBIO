...
def list_locations_by_job(job_id: int) -> QuerySet[JobLocation]:
    ...
    """
    return (
        JobLocation.objects.filter(job_id=job_id)
        .select_related("address", "address__province", "address__commune")
...
def get_job_location_by_id(location_id: int) -> Optional[JobLocation]:
    ...
    try:
        return JobLocation.objects.select_related(
            "address", "address__province", "address__commune", "job", "job__company"
        ).get(id=location_id)
