...
def add_job_location(job: Job, data: JobLocationInput) -> JobLocation:
    ...
    job_location = JobLocation.objects.create(
        job=job, address_id=data.address_id, is_primary=data.is_primary
    )
...
def update_job_location(
    job_location: JobLocation, data: JobLocationInput
) -> JobLocation:
    """
    ...
    if data.address_id != job_location.address_id:
        if JobLocation.objects.filter(
            job=job_location.job, address_id=data.address_id
        ).exists():
            raise ValueError("Địa điểm này đã được thêm cho công việc!")
