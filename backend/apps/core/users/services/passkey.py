...
def _get_attested_credentials(user: CustomUser) -> list:
    ...
    for pk in passkeys:
        ...
        aaguid = (
            bytes.fromhex(pk.aaguid.replace("-", "")) if pk.aaguid else b"\x00" * 16
        )
...
def verify_registration(
    ...
    if not state:
        raise AuthenticationError(
            "Challenge đã hết hạn hoặc không tồn tại. Vui lòng thử lại."
        )
...
def verify_authentication(
    ...
    try:
        ...
        passkey, attested_credentials = _get_all_attested_credentials_by_cred_id(
            cred_id_bytes
        )
