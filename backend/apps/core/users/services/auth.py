    ...
    with transaction.atomic():
        ...
        if profile_provider_id:
            user = CustomUser.objects.filter(
                social_provider=profile_provider, social_id=profile_provider_id
            ).first()
        ...
        if not user:
            ...
            if existing_email_user:
                if (
                    existing_email_user.social_id
                    and existing_email_user.social_id != profile_provider_id
                ):
                    raise AuthenticationError(
                        "Email nay da lien ket voi tai khoan social khac."
                    )
...
def register_user(data: RegisterInput) -> dict:
    ...
    with transaction.atomic():
        ...
        if normalized_role == "company" and data.company_name:
            from apps.company.companies.services.companies import (
                create_company,
                CompanyCreateInput,
            )
            ...
            create_company(
                user=user,
                data=CompanyCreateInput(
                    company_name=data.company_name, tax_code=data.tax_code
                ),
            )
...
def request_set_password(data: RequestSetPasswordInput) -> bool:
    ...
    if user.has_usable_password():
        raise AuthenticationError(
            "Tai khoan da co mat khau. Vui long dung chuc nang doi mat khau."
        )

    if not user.social_provider:
        raise AuthenticationError(
            "Chi tai khoan dang nhap bang social chua co mat khau moi can dat mat khau."
        )
...
def resend_verification(data: ResendVerificationInput) -> bool:
    ...
    verification_link = (
        f"{frontend_base_url}/auth/verify-email?token={user.email_verification_token}"
    )
    ...
    EmailService.send_email(
        ...
        context={"user_name": user.full_name, "verification_link": verification_link},
    )
...
def change_password(data: ChangePasswordInput) -> bool:
    ...
    if not user.has_usable_password():
        raise AuthenticationError(
            "Tai khoan nay chua co mat khau. Vui long dung chuc nang dat mat khau."
        )
...
def _legacy_social_login(data: SocialLoginInput) -> dict:
    ...
    full_name = (
        social_user_data.get("name")
        or social_user_data.get("full_name")
        or data.full_name
    )
...
def enable_2fa(user: CustomUser) -> dict:
    ...
    provisioning_uri = totp.provisioning_uri(name=user.email, issuer_name="JobPortal")
