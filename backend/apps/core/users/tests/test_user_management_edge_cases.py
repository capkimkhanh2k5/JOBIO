...
class TestUpdateUserEdgeCases(APITestCase):
    ...
    def test_update_self_as_normal_user(self):
        ...
        response = self.client.put(
            user_detail(self.normal_user.id), {"full_name": "Updated Name"}
        )
        self.assertIn(
            response.status_code, [status.HTTP_200_OK, status.HTTP_403_FORBIDDEN]
        )
    ...
    def test_update_other_user_as_normal_user(self):
        ...
        self.client.force_authenticate(user=self.normal_user)
        response = self.client.put(
            user_detail(self.target_user.id), {"full_name": "Hacked Name"}
        )
    ...
    def test_update_user_not_found(self):
        ...
        self.client.force_authenticate(user=self.admin)
        response = self.client.put(user_detail(99999), {"full_name": "New Name"})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class TestUpdateStatusEdgeCases(APITestCase):
    ...
    def test_update_status_to_inactive(self):
        ...
        response = self.client.patch(
            user_status(self.target_user.id), {"status": "inactive"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_status_to_active(self):
        ...
        self.client.force_authenticate(user=self.admin)
        response = self.client.patch(
            user_status(self.target_user.id), {"status": "active"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.target_user.refresh_from_db()
    ...
    def test_update_status_invalid_value(self):
        ...
        self.client.force_authenticate(user=self.admin)
        response = self.client.patch(
            user_status(self.target_user.id), {"status": "invalid_status"}
        )
...
class TestUpdateRoleEdgeCases(APITestCase):
    ...
    def test_update_role_to_company(self):
        ...
        self.client.force_authenticate(user=self.admin)
        response = self.client.patch(
            user_role(self.target_user.id), {"role": "company"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.target_user.refresh_from_db()
    ...
    def test_update_role_invalid_value(self):
        ...
        self.client.force_authenticate(user=self.admin)
        response = self.client.patch(
            user_role(self.target_user.id), {"role": "superadmin"}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class TestAvatarEdgeCases(APITestCase):
    ...
    def test_upload_avatar_other_user(self):
        ...
        avatar = SimpleUploadedFile(
            "avatar.jpg", file.read(), content_type="image/jpeg"
        )

        response = self.client.post(
            user_avatar(self.other_user.id), {"avatar": avatar}, format="multipart"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertIn("detail", response.data)
    ...
    def test_upload_non_image_file(self):
        ...
        response = self.client.post(
            user_avatar(self.user.id), {"avatar": text_file}, format="multipart"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_upload_avatar_self(self, mock_upload):
        ...
        file.seek(0)
        avatar = SimpleUploadedFile(
            "my_avatar.jpg", file.read(), content_type="image/jpeg"
        )

        response = self.client.post(
            user_avatar(self.user.id), {"avatar": avatar}, format="multipart"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TestPermissionSecurityFix(APITestCase):
    ...
    def test_admin_can_update_other_user(self):
        ...
        response = self.client.put(
            user_detail(self.target_user.id), {"full_name": "Updated By Admin"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.target_user.refresh_from_db()
    ...
    def test_admin_can_upload_avatar_for_other_user(self, mock_upload):
        ...
        file.seek(0)
        avatar = SimpleUploadedFile(
            "admin_avatar.jpg", file.read(), content_type="image/jpeg"
        )

        response = self.client.post(
            user_avatar(self.target_user.id), {"avatar": avatar}, format="multipart"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class TestBulkActionEdgeCases(APITestCase):
    ...
    def test_bulk_action_as_normal_user(self):
        ...
        response = self.client.post(
            USER_BULK_ACTION,
            {"ids": [1, 2], "action": "update_status", "value": "banned"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_bulk_action_missing_ids(self):
        ...
        response = self.client.post(
            USER_BULK_ACTION,
            {"action": "update_status", "value": "banned"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_bulk_action_missing_action(self):
        ...
        self.client.force_authenticate(user=self.admin)
        response = self.client.post(
            USER_BULK_ACTION, {"ids": [1, 2], "value": "banned"}, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_bulk_action_invalid_action(self):
        ...
        response = self.client.post(
            USER_BULK_ACTION,
            {"ids": [u1.id], "action": "invalid_action", "value": "some_value"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
