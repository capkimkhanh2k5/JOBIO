...
class TestUserManagement(APITestCase):
    ...
    def test_upload_avatar(self, mock_upload):
        ...
        user = CustomUser.objects.create_user(
            email="avatar_user@example.com", password="password"
        )
        self.client.force_authenticate(user=user)

        response = self.client.post(
            user_avatar(user.id), {"avatar": avatar}, format="multipart"
        )
    ...
    def test_get_user_stats(self):
        admin = CustomUser.objects.create_user(
            email="admin_stats@example.com", password="password", role="admin"
        )
        self.client.force_authenticate(user=admin)
    ...
    def test_bulk_limit(self):
        admin = CustomUser.objects.create_user(
            email="admin_bulk@example.com", password="password", role="admin"
        )
        self.client.force_authenticate(user=admin)
        ...
        data = {"ids": [u1.id, u2.id], "action": "update_status", "value": "banned"}
