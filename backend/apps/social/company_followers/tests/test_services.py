...
class CompanyFollowerServiceTest(TestCase):
    def setUp(self):
        ...
        self.user_no_recruiter = CustomUser.objects.create_user(
    ...
    def test_unfollow_company_success(self):
        ...
        self.assertFalse(
            CompanyFollower.objects.filter(
                recruiter=self.recruiter, company=self.company
            ).exists()
        )
        self.company.refresh_from_db()
