...
def get_recruiter_stats(recruiter: Recruiter) -> dict:
    ...
    following_count = (
        recruiter.following_companies.count()
        if hasattr(recruiter, "following_companies")
        else 0
    )
...
def search_recruiters(filters: dict) -> QuerySet:
    ...
    """
    queryset = Recruiter.objects.select_related(
    ...
    if search_query and search_query.lower() != "all":
        ...
        search_vector = SearchVector(
            "current_position", weight="A", config="english"
    ...
    if location and location.lower() != "all":
        queryset = queryset.filter(address__province__province_name__icontains=location)
