...
from .selectors.recruiter_skills import list_skills_by_recruiter, get_skill_by_id
...
class RecruiterSkillViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request, recruiter_id=None):
        ...
        try:
            ...
            return Response(
                RecruiterSkillSerializer(skill).data, status=status.HTTP_201_CREATED
            )
    ...
    def bulk_add(self, request, recruiter_id=None):
        ...
        try:
            skills_data = [
                SkillInput(**item)
                for item in serializer.validated_data.get("skills", [])
            ]
