...
@transaction.atomic
def create_education_service(
    recruiter: Recruiter, data: EducationInput
) -> RecruiterEducation:
    """
    ...
    max_order = RecruiterEducation.objects.filter(recruiter=recruiter).aggregate(
        Max("display_order")
    )["display_order__max"]
...
@transaction.atomic
def update_education_service(
    education: RecruiterEducation, data: EducationInput
) -> RecruiterEducation:
    """
...
def reorder_education_service(recruiter: Recruiter, order_data: list) -> None:
    ...
    valid_ids = set(
        RecruiterEducation.objects.filter(recruiter=recruiter).values_list(
            "id", flat=True
        )
    )
    ...
    for item in order_data:
        if item["id"] not in valid_ids:
            raise ValueError(
                f"Education id {item['id']} does not belong to this recruiter"
            )
