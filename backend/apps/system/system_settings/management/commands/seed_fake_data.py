...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        self.stdout.write(
            self.style.WARNING("Đang bắt đầu tạo dữ liệu mẫu... Bắt đầu transaction.")
        )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS("=> SEED FAKE DATA HOÀN TẤT THÀNH CÔNG!")
            )
    ...
    def _seed_users(self, count):
        ...
        self.stdout.write(
            f"Đã tạo {len(created_users)} users đóng vai trò nhà tuyển dụng."
        )
    ...
    def _seed_companies(self, users):
        ...
        if not industries:
            self.stdout.write(
                self.style.ERROR(
                    "Chưa có Industry nào. Vui lòng chạy lệnh seed_master_data trước."
                )
            )
            return []
            ...
            companies.append(
                Company(
                    ...
                    company_size=random.choice(
                        [choice[0] for choice in Company.CompanySize.choices]
                    ),
