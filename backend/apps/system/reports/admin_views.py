...
from .serializers import AdminReportSerializer, AdminReportStatusUpdateSerializer, ReportResolutionSerializer
...
class AdminReportViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def resolve(self, request, pk=None):
        ...
        if report.status in [Report.Status.RESOLVED, Report.Status.REJECTED]:
            return Response({"detail": "Báo cáo đã được xử lý."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            if action_type == 'ban':
                ...
                if entity_job:
                    ...
                    entity_company.jobs.filter(status=Job.Status.PUBLISHED).update(status=Job.Status.CLOSED)
                    if entity_user:
            ...
            logger.error(f"Error applying action {action_type} to entity {report.entity_type} {report.entity_id}: {e}")

        # Thông báo cho người báo cáo (Reporter)
        if report.reporter and report.reporter.email:
            action_text = "đã từ chối do không phát hiện vi phạm" if action_type == 'reject' else "đã tiến hành xử lý nghiêm"
            
            reporter_body = render_to_string('emails/report/reporter.html', {
                'full_name': report.reporter.full_name or report.reporter.email,
                'report_type_name': report.report_type.type_name if report.report_type else 'Vi phạm',
                'report_id': report.id,
        ...
        if entity_email and action_type != 'reject':
            ...
            extra_text = "Vui lòng khắc phục ngay lập tức để tránh các hình thức xử lý nặng hơn."
            ...
            if action_type == 'ban':
                ...
                extra_text = "Vui lòng liên hệ bộ phận hỗ trợ nếu bạn có bất kỳ thắc mắc nào."

            violator_body = render_to_string('emails/report/report_result_violator.html', {
                'report_type_name': report.report_type.type_name if report.report_type else 'Vi phạm',
