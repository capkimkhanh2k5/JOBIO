...
INDUSTRY_COLORS = [
    '#7c3aed', '#a78bfa', '#f97316', '#fb923c', '#c084fc',
...
def revenue_trend(request):
    ...
    qs = (
        Transaction.objects
        .filter(status=Transaction.Status.COMPLETED, created_at__gte=start)
...
def application_stats(request):
    ...
    reviewing_apps = Application.objects.filter(status=Application.Status.REVIEWING).count()
    shortlisted_apps = Application.objects.filter(status=Application.Status.SHORTLISTED).count()
    interview_apps = Application.objects.filter(status=Application.Status.INTERVIEW).count()
    offered_apps = Application.objects.filter(status=Application.Status.OFFERED).count()
    accepted_apps = Application.objects.filter(status=Application.Status.ACCEPTED).count()
    rejected_apps = Application.objects.filter(status=Application.Status.REJECTED).count()
    ...
    scheduled_interviews = Interview.objects.filter(status=Interview.Status.SCHEDULED).count()
    completed_interviews = Interview.objects.filter(status=Interview.Status.COMPLETED).count()
    cancelled_interviews = Interview.objects.filter(status=Interview.Status.CANCELLED).count()
    ...
    # Status breakdown
    status_breakdown = [
        {'status': 'pending', 'label': 'Chờ xử lý', 'count': pending_apps, 'color': '#f59e0b'},
        {'status': 'reviewing', 'label': 'Đang xem xét', 'count': reviewing_apps, 'color': '#6366f1'},
        {'status': 'shortlisted', 'label': 'Vòng tiếp', 'count': shortlisted_apps, 'color': '#a78bfa'},
        {'status': 'interview', 'label': 'Phỏng vấn', 'count': interview_apps, 'color': '#f97316'},
        {'status': 'accepted', 'label': 'Nhận việc', 'count': accepted_apps, 'color': '#10b981'},
        {'status': 'rejected', 'label': 'Từ chối', 'count': rejected_apps, 'color': '#ef4444'},
    ]
...
def violation_breakdown(request):
    ...
    qs = (
        Report.objects
        .filter(report_type__isnull=False)
...
def admin_overview_stats(request):
    ...
    return Response({
        'users': {
            ...
            'new_30d': CustomUser.objects.filter(date_joined__gte=thirty_ago).count(),
            'by_role': {
