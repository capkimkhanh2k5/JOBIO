...
class CompanyMediaViewSet(viewsets.ViewSet):
    ...
    def create(self, request, company_id=None):
        ...
        try:
            ...
            media = upload_company_media_service(
                company_id=company_id, user=request.user, data=input_data
            )
            return Response(
                CompanyMediaSerializer(media).data, status=status.HTTP_201_CREATED
            )
    ...
    def update(self, request, company_id=None, pk=None):
        ...
        try:
            ...
            media = update_company_media_service(
                media_id=pk, user=request.user, data=input_data
            )
            return Response(
                CompanyMediaSerializer(media).data, status=status.HTTP_200_OK
            )
        except ValueError as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def destroy(self, request, company_id=None, pk=None):
        """Xóa media"""
        try:
            delete_company_media_service(media_id=pk, user=request.user)
    ...
    def reorder(self, request, company_id=None):
        ...
        try:
            input_data = CompanyMediaReorderInput(
                reorders=serializer.validated_data["reorders"]
            )
            reorder_company_media_service(
                company_id=company_id, user=request.user, data=input_data
            )
            return Response(
                {"message": "Sắp xếp thành công"}, status=status.HTTP_200_OK
            )
        except ValueError as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
