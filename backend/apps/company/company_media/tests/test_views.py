...
class CompanyMediaViewTest(TestCase):
    ...
    def test_create_media(self, mock_save):
        ...
        response = self.client.post(
            f"/api/companies/{self.company.id}/media", data, format="multipart"
        )
    ...
    def test_update_media(self):
        ...
        response = self.client.put(
            f"/api/companies/{self.company.id}/media/{media.id}",
            data,
            format="multipart",
        )
    ...
    def test_reorder_media(self):
        ...
        data = {"reorders": [{"id": m1.id, "display_order": 5}]}

        response = self.client.patch(
            f"/api/companies/{self.company.id}/media/reorder", data, format="json"
        )
