...
class BaseCompanyTestCase(TestCase):
    def setUp(self):
        ...
        self.company_user = CustomUser.objects.create_user(
            email="companyowner@example.com", password="password123", role="company"
        )
        self.other_user = CustomUser.objects.create_user(
            email="other@example.com", password="password123"
        )
        self.company = Company.objects.create(
            user=self.company_user, company_name="Test Company", slug="test-company"
        )
...
class TestUploadLogo(BaseCompanyTestCase):
    ...
    def test_upload_logo_success(self, mock_upload):
        ...
        response = self.client.post(
            company_logo(self.company.id), {"logo": logo}, format="multipart"
        )
    ...
    def test_upload_logo_no_file(self):
        ...
        response = self.client.post(
            company_logo(self.company.id), {}, format="multipart"
        )
    ...
    def test_upload_logo_company_not_found(self):
        ...
        response = self.client.post(
            company_logo(99999), {"logo": logo}, format="multipart"
        )
...
class TestUploadBanner(BaseCompanyTestCase):
    ...
    def test_upload_banner_success(self, mock_upload):
        ...
        banner = SimpleUploadedFile(
            "banner.jpg", file.read(), content_type="image/jpeg"
        )

        response = self.client.post(
            company_banner(self.company.id), {"banner": banner}, format="multipart"
        )
...
class TestCompanySuggestions(TestCase):
    ...
    def _setup_skill_matching(self):
        ...
        python_skill = Skill.objects.create(
            name="Python", slug="python", category=category
        )
    ...
    def test_suggestions_fallback(self):
        ...
        no_skill_user = CustomUser.objects.create_user(
            email="no_skill@example.com", password="password"
        )
        from apps.candidate.recruiters.models import Recruiter
...
class TestJobStatusFiltering(TestCase):
    ...
    def setUp(self):
        ...
        self.company = Company.objects.create(
            company_name="My Company", slug="my-company", user=self.user
        )
