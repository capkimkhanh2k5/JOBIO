...
class CompanySuggestionService:
    ...
    def get_suggestions(user, limit=10):
        ...
        user_skill_ids = RecruiterSkill.objects.filter(recruiter=recruiter).values_list(
            "skill_id", flat=True
        )
        ...
        suggested_companies = (
            ...
            .annotate(
                match_count=Count(
                    "jobs__required_skills",
                    filter=Q(jobs__required_skills__skill_id__in=user_skill_ids),
                )
            )
            .order_by(
        ...
        if len(suggested_companies) < 3:
            ...
            top_trending = base_qs.exclude(id__in=exclude_ids).order_by(
                "-follower_count"
            )[: limit - len(suggested_companies)]
