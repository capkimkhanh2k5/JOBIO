...
class BenefitCategoryViewSetTests(TestCase):
    ...
    def setUp(self):
        ...
        self.category2 = BenefitCategory.objects.create(
            name="Paid Time Off", slug="paid-time-off", is_active=True, display_order=1
        )
        self.category3 = BenefitCategory.objects.create(
            name="Remote Work", slug="remote-work", is_active=False, display_order=2
        )
    ...
    def test_update_category_admin(self):
        ...
        response = self.client.put(
            f"/api/benefit-categories/{self.category1.id}/", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_reorder_categories(self):
        ...
        response = self.client.patch(
            "/api/benefit-categories/reorder/", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_reorder_unauthorized(self):
        ...
        self.client.force_authenticate(user=self.user)
        data = {"items": [{"id": self.category1.id, "display_order": 1}]}
        response = self.client.patch(
            "/api/benefit-categories/reorder/", data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_reorder_invalid_id(self):
        ...
        self.client.force_authenticate(user=self.admin)
        data = {"items": [{"id": 9999, "display_order": 0}]}
        response = self.client.patch(
            "/api/benefit-categories/reorder/", data, format="json"
        )
