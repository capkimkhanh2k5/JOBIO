...
class TestCompanyBenefitsAPIs(APITestCase):
    def setUp(self):
        ...
        self.company = Company.objects.create(
            user=self.company_owner, company_name="Test Company", slug="test-company"
        )
    ...
    def test_create_benefit_not_owner(self):
        ...
        data = {"category_id": self.category.id, "benefit_name": "Test"}
    ...
    def test_create_benefit_invalid_category(self):
        ...
        self.client.force_authenticate(user=self.company_owner)
        data = {"category_id": 9999, "benefit_name": "Test"}
        response = self.client.post(benefits_list(self.company.id), data, format="json")
    ...
    def test_update_benefit_success(self):
        ...
        self.client.force_authenticate(user=self.company_owner)
        data = {"benefit_name": "Updated Benefit", "description": "Updated description"}
        response = self.client.put(
            benefit_detail(self.company.id, self.benefit.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_reorder_benefits_success(self):
        ...
        # Create b2, b3
        b2 = CompanyBenefit.objects.create(
            company=self.company,
            category=self.category,
            benefit_name="B2",
            display_order=2,
        )
        b3 = CompanyBenefit.objects.create(
            company=self.company,
            category=self.category,
            benefit_name="B3",
            display_order=3,
        ...
        data = {"benefit_ids": [b3.id, self.benefit.id, b2.id]}
        response = self.client.patch(
            benefits_reorder(self.company.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_reorder_benefits_not_owner(self):
        ...
        self.client.force_authenticate(user=self.other_user)
        data = {"benefit_ids": [self.benefit.id]}
        response = self.client.patch(
            benefits_reorder(self.company.id), data, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
