...
def list_benefits_by_company(company_id: int):
    ...
    """
    return CompanyBenefit.objects.filter(company_id=company_id).select_related(
        "category"
    )
...
def get_benefit_by_id(benefit_id: int):
    ...
    try:
        return CompanyBenefit.objects.select_related("category", "company").get(
            id=benefit_id
        )
...
def get_benefit_ids_by_company(company_id: int) -> set:
    ...
    return set(
        CompanyBenefit.objects.filter(company_id=company_id).values_list(
            "id", flat=True
        )
    )
...
def get_max_display_order(company_id: int) -> int:
    ...
    result = CompanyBenefit.objects.filter(company_id=company_id).aggregate(
        Max("display_order")
    )
