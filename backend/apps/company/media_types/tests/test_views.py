...
class MediaTypeViewSetTests(APITestCase):
    def setUp(self):
        ...
        # Create media types
        self.image = MediaType.objects.create(
            type_name="Image", description="Image files", is_active=True
        )
        self.video = MediaType.objects.create(
            type_name="Video", description="Video files", is_active=True
        )
        self.doc = MediaType.objects.create(
            type_name="Document", description="Document files", is_active=False
        )
    ...
    def test_update_media_type_duplicate_name_fails(self):
        ...
        data = {
            "type_name": self.image.type_name
        }  # Try to use 'Image' which belongs to another object
