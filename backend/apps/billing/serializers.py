...
class CompanySubscriptionSerializer(serializers.ModelSerializer):
    ...
    def get_usage(self, obj):
        ...
        active_jobs_count = Job.objects.filter(
            company=obj.company, status="published"
        ).count()

        # Count featured jobs
        featured_jobs_count = Job.objects.filter(
            company=obj.company, status="published", featured=True
        ).count()
        ...
        return {
            "jobs": {
                ...
                "limit": self._feature_value(
                    features, "job_post_limit", "max_jobs", "max_job_posts"
                ),
            },
            "featured_jobs": {
                "current": featured_jobs_count,
                "limit": self._feature_value(
                    features, "featured_job_limit", "max_featured_jobs"
                ),
            },
            "cv_views": {
                "current": 0,  # Not tracked yet
                "limit": self._feature_value(
                    features, "cv_view_limit", "max_cv_views", "max_cv_view"
                ),
            },
            "ai_matching": {
                "enabled": self._feature_value(
                    features, "has_ai_matching", "ai_matching", default=False
                )
            },
        }
...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    def get_plan_name(self, obj):
        ...
        if isinstance(metadata, dict) and metadata.get("plan_id"):
            return (
                SubscriptionPlan.objects.filter(id=metadata.get("plan_id"))
