...
import { Eye, Clock, User, Calendar, GripVertical, FileText } from 'lucide-react';
...
export function CandidateBoard({ applications, isLoading, onStatusChange }: {
    ...
    const handleDragStart = (e: React.DragEvent, id: string) => {
        ...
        // Needs a transparent image or we can just let browser handle it
        setTimeout(() => {
    ...
    if (isLoading) {
        return (
            <div className="flex gap-4 overflow-x-auto pb-4 h-[calc(100vh-280px)]">
                {COLUMNS.map(col => (
                    <div key={col} className="w-[300px] flex-shrink-0 bg-secondary/30 rounded-xl p-3 border border-border/50 animate-pulse h-full">
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center justify-between text-xs pt-3 border-t border-border/50">
                                            <div className="flex items-center gap-1.5 text-muted-foreground">
                                                ...
                                                <span>{new Date(app.applied_at).toLocaleDateString('vi-VN', { month: 'short', day: 'numeric' })}</span>
                                            </div>
                                            <div className={cn("flex items-center gap-1 px-1.5 rounded-md border text-[10px] font-medium", getScoreColor(app.ai_score))}>
