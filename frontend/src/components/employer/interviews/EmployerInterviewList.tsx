...
export function EmployerInterviewList({ interviews, isLoading, onInterviewClick }: EmployerInterviewListProps) {
    const getStatusTextAndColor = (status: Interview['status']) => {
        switch (status) {
            case 'scheduled': return { text: 'Sắp tới', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-800' };
            case 'confirmed': return { text: 'Đã xác nhận', color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border-green-200 dark:border-green-800' };
            case 'completed': return { text: 'Hoàn thành', color: 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400 border-slate-200 dark:border-slate-700' };
            case 'cancelled': return { text: 'Đã hủy', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border-red-200 dark:border-red-800' };
            case 'no_show': return { text: 'Vắng mặt', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 border-red-200 dark:border-red-800' };
            case 'in_progress': return { text: 'Đang diễn ra', color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400 border-amber-200 dark:border-amber-800' };
        ...
        return (
            <div className="w-full">
                <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
                    <div className="bg-slate-50 dark:bg-slate-800/50 min-h-[50px] border-b border-slate-200 dark:border-slate-800"></div>
                    {Array(5).fill(null).map((_, i) => (
                        <div key={i} className="flex p-4 border-b border-slate-200 dark:border-slate-800 gap-4">
    ...
    if (!interviews || interviews.length === 0) {
        return (
            <div className="p-12 text-center text-slate-500 border border-slate-200 dark:border-slate-800 rounded-xl">
                <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4">
                    <Video className="w-8 h-8 text-slate-400" />
                </div>
                <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">Không có lịch phỏng vấn nào</h3>
    ...
    return (
        <div className="w-full border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900 shadow-sm">
            <div className="overflow-x-auto min-h-[600px]">
                <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 font-medium">
                        <tr>
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                        {interviews.map((interview) => {
                            ...
                            return (
                                <tr key={interview.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors group">
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-[160px] bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
                                                ...
                                                {interview.status !== 'cancelled' && interview.status !== 'completed' && (
                                                    <DropdownMenuItem className="text-red-600 focus:bg-red-50 focus:text-red-700 dark:text-red-500 dark:focus:bg-red-900/30 cursor-pointer">
            ...
            {interviews.length > 0 && (
                <div className="py-4 px-6 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-sm text-slate-500">
                    <div>Hiển thị <span className="font-medium text-slate-900 dark:text-white">1</span> đến <span className="font-medium text-slate-900 dark:text-white">{interviews.length}</span> trong <span className="font-medium text-slate-900 dark:text-white">{interviews.length}</span> kết quả</div>
