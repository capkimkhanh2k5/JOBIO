...
import { CVSearchFiltersPanel } from '@/components/company/cv/CVSearchFilters';
import { CandidateCard, CandidateCardSkeleton } from '@/components/company/cv/CandidateCard';
import { CandidateProfileSheet } from '@/components/company/cv/CandidateProfileSheet';
import { PageHeader } from '@/components/shared/PageHeader';

export default function CompanyCVSearch() {
    const filters = useFilterStore(state => state.cvFilters);
    const [selectedCandidateId, setSelectedCandidateId] = useState<string | null>(null);
    const navigate = useNavigate();

    const { data, isLoading, isFetching } = useQuery({
        queryKey: ['company', 'cvSearch', filters, 1],
    ...
    const handleCreateJob = () => navigate('/company/jobs/create');
    ...
    return (
        <div className="w-full mx-auto min-h-screen flex flex-col">
            ...
            <CandidateProfileSheet
                ...
                onOpenChange={(open: boolean) => !open && setSelectedCandidateId(null)}
