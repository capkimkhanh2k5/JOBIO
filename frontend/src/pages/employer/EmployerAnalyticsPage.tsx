...
import {
    ...
    BarChart3, Download, RefreshCw, CalendarDays, Target, Percent,
...
function SummaryKpiCard({
    ...
    return (
        <motion.div
            ...
            className="bg-white border border-slate-200 shadow-sm hover:shadow-md rounded-2xl p-5 relative overflow-hidden group"
        >
...
// ─── Custom Tooltip for bar (funnel) ─────────────────────────────────────────
function BarTooltip({ active, payload }: any) {
    if (!active || !payload?.length) return null;
    const item = payload[0];
    return (
        <div className="bg-white rounded-xl px-4 py-3 border border-slate-200 shadow-xl text-sm">
            <p className="font-bold text-slate-900 mb-1">{item.payload.stage}</p>
            <p className="text-slate-600 font-semibold">{item.value} ứng viên</p>
        </div>
    );
}
        ...
        >
            <td className="px-5 py-4">
                <div className="flex items-center gap-3">
                    ...
                    <Link to={`/employer/jobs/${job.id}`} className="font-semibold text-slate-800 hover:text-violet-600 transition-colors line-clamp-1 text-sm">
                        {job.title}
                    </Link>
                </div>
            </td>
...
export function EmployerAnalyticsPage() {
    ...
    const kpiCards = [
        ...
        },
        {
            ...
            note: `${summary?.hiring_rate ?? summary?.hire_rate ?? 0}% tỷ lệ tuyển dụng`,
        },
        {
            ...
            <motion.div {...fadeUp(0)} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1 font-medium">
                        Theo dõi hiệu quả tuyển dụng và đưa ra quyết định dựa trên dữ liệu
                    </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                    <Button
                        ...
                        onClick={() => refetch()}
                        disabled={isFetching}
                        className="gap-2 text-slate-600 border-slate-200 hover:border-violet-200 hover:text-violet-600 cursor-pointer"
                                        ...
                                        <tr>
                                            <td colSpan={7} className="px-5 py-12 text-center">
                                                <div className="flex flex-col items-center text-slate-400">
                                                    ...
                                                    <Link to="/employer/jobs/create" className="mt-3 text-violet-600 text-sm font-semibold hover:underline" id="analytics-post-job-link">
                                                        Đăng tin ngay →
                                                    </Link>
                                                </div>
                                            </td>
