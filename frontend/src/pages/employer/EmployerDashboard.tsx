...
export default function EmployerDashboard() {
    ...
    const quickActions = [
        { icon: <PlusSquare className="w-6 h-6" />, label: 'Đăng tin mới', to: '/employer/jobs/create', gradient: 'from-violet-500 to-indigo-600' },
        { icon: <Search className="w-6 h-6" />, label: 'Tìm CV', to: '/employer/cv-search', gradient: 'from-indigo-500 to-violet-600' },
        { icon: <User2 className="w-6 h-6" />, label: 'Xem ứng viên', to: '/employer/candidates', gradient: 'from-violet-400 to-indigo-500' },
    ];
                ...
                <motion.div {...fadeUp(0.22)} className="xl:col-span-1 h-full">
                    <div className="bg-white rounded-3xl p-8 flex flex-col gap-6 border border-slate-200 shadow-sm h-full">
                        ...
                        <div className="flex flex-col gap-1.5 flex-1">
                            {[
                                { label: '📋 Đăng tin mới', to: '/employer/jobs/create', desc: 'Tạo tin ngay' },
                                { label: '🔍 Tìm CV', to: '/employer/cv-search', desc: 'Khám phá hồ sơ' },
                                { label: '👥 Xem ứng viên', to: '/employer/candidates', desc: 'Quản lý danh sách' },
                                { label: '📊 Báo cáo', to: '/employer/analytics', desc: 'Phân tích hiệu quả' },
                                { label: '⚙️ Cài đặt', to: '/employer/settings', desc: 'Tùy chỉnh' },
                            ].map((item) => (
                                <Link
                                    key={item.to}
                                    to={item.to}
