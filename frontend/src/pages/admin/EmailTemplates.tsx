            ...
            {activeTab === 'templates' && (
                <motion.div {...fadeUp(0.1)} className="space-y-4">
                    {loadingTemplates ? (
                        ...
                        <div key={tpl.id} className="bg-white/60 backdrop-blur-xl rounded-2xl border border-white/40 shadow-sm p-5 hover:shadow-md transition-shadow">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex flex-wrap gap-1 mt-2">
                                        {(tpl.variables ?? []).map(v => (
                                            <code key={v} className="text-[10px] bg-violet-50 text-violet-600 px-1.5 py-0.5 rounded-md font-mono">{`{{ ${v} }}`}</code>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div key={cat.id} className="bg-white/60 backdrop-blur-xl rounded-2xl border border-white/40 shadow-sm p-5 hover:shadow-md transition-shadow group cursor-pointer">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-xl bg-violet-50 border border-violet-100/50 flex items-center justify-center"><FolderOpen className="w-5 h-5 text-violet-600" /></div>
                                        <div>
                                    ...
                                    <button className="p-1 rounded-lg hover:bg-slate-100/50 text-slate-400 opacity-0 group-hover:opacity-100 transition-all cursor-pointer"><Edit3 className="w-4 h-4" /></button>
                                </div>
                            </div>
                        ))}
                        <button className="bg-white/40 backdrop-blur rounded-2xl border-2 border-dashed border-slate-200 p-5 flex items-center justify-center gap-2 text-slate-400 hover:border-violet-300 hover:bg-violet-50/30 hover:text-violet-600 transition-all cursor-pointer">
